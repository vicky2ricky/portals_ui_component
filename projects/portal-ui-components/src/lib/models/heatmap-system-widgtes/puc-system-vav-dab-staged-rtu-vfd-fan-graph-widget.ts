...
/**
 * @description System VAV/DAB Staged RTU with VFD Fan Graph Widget
 */
export class PucSystemVavDabStagedRtuVfdFanGraphWidget {
    constructor() { }
        ...
        parameters.forEach(param => {
            if (param === 'humidifier') {
                ...
                params.push(new PucGraphWidgetParam('humidifierOn', PucGraphColors.LIGHT_CYAN,
                    PucGraphTypes.HORIZONTALLY_STACKED_SINGULAR, toolTip, isFac ? 5 : 0));
                // off
                ...
                // on
                toolTip = new PucToolTipModel(true, 'Dehumidifier', true, 'On'); // stacked-merged
                params.push(new PucGraphWidgetParam('dehumidifierOn', PucGraphColors.MOSTLY_PURE_BLUE,
                    PucGraphTypes.HORIZONTALLY_STACKED_SINGULAR, toolTip, isFac ? 5 : 0));
                // off
                toolTip = new PucToolTipModel(true, 'Dehumidifier', true, 'Off'); // stacked-merged
                params.push(new PucGraphWidgetParam('dehumidifierOff', PucGraphColors.GREY,
                    PucGraphTypes.HORIZONTALLY_STACKED_SINGULAR, toolTip, isFac ? 5 : 0));
            } else {
                toolTip = this.getParamToolTip(param);
    ...
    getParamGraphType(param: string) {
        let type;
        switch (param) {
            case 'fanStage1': case 'fanStage2': case 'fanStage3': case 'fanStage4': case 'fanStage5': case 'fanSignal':
    ...
    getParamGraphColor(param: string) {
        ...
        switch (param) {
            ...
            case 'coolingStage2':
                ...
                break;
            case 'coolingStage3':
                color = PucGraphColors.SOFT_BLUE;
                break;
            case 'coolingStage4':
    ...
    getParamToolTip(param: string) {
        let tooltip: PucToolTipModel;
